...
import { createProductShcema } from "../../lib/schemas/createProductSchema"
...
export default function ProductForm({ setEditMode, product, refetch, setSelectedProduct }: Props) {
    const { control, handleSubmit, watch, reset, setError, formState: { isSubmitting } } = useForm<createProductShcema>({
        mode: 'onTouched',
        resolver: zodResolver(createProductShcema),
    })
    ...
    const onSubmit = async (data: createProductShcema) => {
        try {
            ...
            console.log(error);
            handleApiError<createProductShcema>(error, setError,
